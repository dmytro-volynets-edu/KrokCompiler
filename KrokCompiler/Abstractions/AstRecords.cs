...
namespace KrokCompiler.Abstractions
{
    ...
    public record NopStmt(string Name) : Stmt
    {
        public override void Accept(IAstVisitor visitor) => visitor.VisitNopStmt(this);
    }

    /// <summary>
    /// Допоміжний вузол для друку списків (напр. "Arguments").
    /// </summary>
    public record AstListNode(string Name, List<IAstNode> Children) : IAstNode
    {
        // Конструктори для сумісності зі списками Expr, Stmt
        ...
        public void Accept(IAstVisitor visitor) => visitor.VisitAstListNode(this);
    }
}
